...
export class SongService {
  ...
  update(song: ISong): Observable<EntityResponseType> {
    return this.http.put<ISong>(`${this.resourceUrl}/${getSongIdentifier(song) as string}`, song, { observe: 'response' });
  }

  partialUpdate(song: ISong): Observable<EntityResponseType> {
    return this.http.patch<ISong>(`${this.resourceUrl}/${getSongIdentifier(song) as string}`, song, { observe: 'response' });
  }
